...
$query="select dmn_id  from estt_dmn_mstr where dmn_dscrptn= replace('$dmnidrltn',' and ',' & ')";
 ...
 $query="select dmn_id, replace(dmn_dscrptn,'&','and') DMN_DSCRPTN  from estt_dmn_mstr where nvl(dmn_id_rltn,'%') like '$dmnidrltn' and dmn_typ like '$dmntyp' order by replace(dmn_dscrptn,'&','and')";
 $statemen=oci_parse($conn,$query);
 oci_execute($statemen);
 ...
 $wingcd=$row["DMN_ID"];
 $respotext.="<option value='$wingcd'>$wing</option>";
