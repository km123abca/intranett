...
<a href="empreports.php" >back to main page</a>
    <fieldset>
...
<tr> 
    ...
    <th><button type="button" onclick="rearrange('p2')">Deputed from</button></th> 
...
$query=" select ps_nm||','||cdr.dmn_dscrptn nd,
                ps_room_no hq,
                b.dmn_dscrptn place,
                SR_DOE_DPTTN_TO_CAG  frd,
                nvl(to_char(SR_DPTTN_TO_CAG_SNC_DT,'dd-mm-yyyy'),to_char(sysdate-1,'dd-mm-yyyy'))  ttd,
                ' ' remarks
          from  prsnl_infrmtn_systm a,
                estt_dmn_mstr b,
                estt_dmn_mstr cdr,
                estt_dmn_mstr stat 
          where ((PS_DPTTN_FR_PLACE is not null) or(lower(cdr.dmn_dscrptn) like '%deptn%')
                or (lower(cdr.dmn_dscrptn) like '%legal%') or 
                (lower(cdr.dmn_dscrptn) like '%welfare%assis%') or
                (lower(cdr.dmn_dscrptn) like '%data manager%') or
                (lower(cdr.dmn_dscrptn) like '%data pro%')      )
          and b.dmn_id(+)=PS_DPTTN_FR_PLACE and stat.dmn_dscrptn='WORKING'
...
$ordar=array('p0'=>"ps_cdr_id||','||ps_nm",
             ...
             'p2'=>'b.dmn_dscrptn',
             'p3'=>'SR_DOE_DPTTN_TO_CAG ',
             'p4'=>'SR_DPTTN_TO_CAG_SNC_DT '
             );
