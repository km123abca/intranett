...
<fieldset class="lg">
	...
	<lab class="lg bl">Branch</lab>   
	      ...
	      <option value='%' selected>select all</option>
          		<?php
          		$drel=$fgmembersite->SafeDisplay('wing');
          		if ($drel=='') $drel='%';
          		$query="select dmn_id, dmn_dscrptn from estt_dmn_mstr where dmn_typ='03'
          		         and dmn_id_rltn like '$drel'  order by dmn_dscrptn";
	      		...
	      		if ($fgmembersite->SafeDisplay('branch')==$val)  
	      			echo "<option value='$val' selected>$dumval</option>";
	      		else
	...
	<lab class="lg bl">Section</lab>   
	      ...
	      <option value='%' selected>select all</option>
          		<?php
          		$drel=$fgmembersite->SafeDisplay('branch');
          		if ($drel=='') $drel='%';
	      		...
	      		$val=$row["DMN_ID"];
	      		$dumval=$row["DMN_DSCRPTN"];
	      		if ($fgmembersite->SafeDisplay('section')==$val)  
	      			echo "<option value='$val' selected>$dumval</option>";
	      		else
	      			echo "<option value='$val'>$dumval</option>";
	      				}
	 ...
	 <lab class="lg bl">Designation</lab>   
	      ...
	      <option value='%' selected>select all</option>
          		<?php
          		$query="select dmn_id, dmn_dscrptn from estt_dmn_mstr where dmn_typ='01' order by dmn_dscrptn";
          		$statemen=oci_parse($conn,$query);
		  		oci_execute($statemen);
	      		while( $row=oci_fetch_array($statemen))
	      				{
	      		$val=$row["DMN_ID"];
	      		$dumval=$row["DMN_DSCRPTN"];
	      		if ($fgmembersite->SafeDisplay('cadre')==$val)  
	      			echo "<option value='$val' selected>$dumval</option>";
	      		else
	      			echo "<option value='$val'>$dumval</option>";
	      				}
...
<?php
$selval=$fgmembersite->SafeDisplay('sel');
if ($selval=='') $selval='ps_wing';
?>
<script>
var selval=<?php echo "'".$selval."'"; ?>;
for( var ind in document.getElementsByName("sel"))
		{
		if (document.getElementsByName("sel")[ind].value==selval)
		document.getElementsByName("sel")[ind].checked=true;
		else
		document.getElementsByName("sel")[ind].checked=false;
		}
</script>
...
<fieldset class="lg">
	...
	<lab class="flg"><input  type='radio' id='cntt1' name='csel' value='ps_wing' 
	                 onfocus="makeviz(this.value) " >wings</lab>
	<lab class="flg"><input  type='radio' id='cntt2' name='csel' value='ps_brnch_id' 
					 onfocus="makeviz(this.value) " checked>branches</lab>
...
<?php
$selval=$fgmembersite->SafeDisplay('csel');
if ($selval=='') $selval='ps_brnch_id';
?>
<script>
var selval=<?php echo "'".$selval."'"; ?>;
for( var ind in document.getElementsByName("csel"))
		{
		if (document.getElementsByName("csel")[ind].value==selval)
		document.getElementsByName("csel")[ind].checked=true;
		else
		document.getElementsByName("csel")[ind].checked=false;
		}
</script>
...
if ($tobecnt=='ps_cdr_id') $tobecnt='ps_idn';
...
if (($tobecnt=="")||($per_crt=="")) die("<red style='color:red;'>what do you want to be counted?</red>");

$query="select $per_crt col,count($tobecnt) cnt,dmn_dscrptn from 
        (select distinct $per_crt,$tobecnt,dmn_dscrptn
        ...
        where ps_wing like '$wing' and 
              ...
              ps_room_no like '$hq' and dmn_id(+)=$per_crt) aa
              group by $per_crt,dmn_dscrptn 
              order by $per_crt";
...
echo "<tr>";
     ...
     $desc2=($tobecnt=='ps_idn')?$desc:$code2desc[$tobecnt];
     echo "<th>Number of $desc2"."s</th>";
echo "</tr>";
